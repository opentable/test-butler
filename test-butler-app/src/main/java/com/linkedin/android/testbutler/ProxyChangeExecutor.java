...
public class ProxyChangeExecutor extends BroadcastReceiver {
    ...
    public ProxyChangeExecutor(Context context) {
     ...
     */
    public boolean executeChange(ProxyChangeParams proxyChangeParams) {
        ...
        if (proxyChangeParams.getNetworkId() != null) {
            ...
            return executeProxyChange(proxyChangeParams);
        } else {
    ...
    public boolean validateIntent(Intent intent) {
        ...
        if (!intent.hasExtra(ProxyChangeParams.HOST) && !intent.hasExtra(ProxyChangeParams.CLEAR)) {
            Log.e(TAG, "Error: No HOST given or not clearing proxy");
            return false;
        }
        if (!intent.hasExtra(ProxyChangeParams.SSID)) {
            Log.e(TAG, "Warning: No SSID given, setting on the fist one available");
        }
        return true;
    }
    ...
    private APLNetworkId findNetworkId(ProxyChangeParams params) {
        ...
        if (params.getSsid() == null) {
            Log.d(TAG, "No SSID specified - trying to find connected.");
            for (Map.Entry<APLNetworkId, WifiConfiguration> entry : networks.entrySet()) {
                if (entry.getValue().status == WifiConfiguration.Status.CURRENT) {
                    Log.d(TAG, String.format("No SSID specified, but found %s as the connected network - settings proxy on this!",
                            entry.getKey().SSID));
                    return entry.getKey();
                }
            }
    ...
    private boolean executeProxyChange(ProxyChangeParams params) {
        if (!setProxy(params)) {
            if (params.isResetWifi()) {
                Log.d(TAG, "Error: proxy not set. Trying to reset wifi and set again.");
                clearAndReconnectWifi(params);
                if (!setProxy(params)) {
    ...
    private boolean setProxy(ProxyChangeParams params) {
        ...
        if (newConfig != null && newConfig.getProxySetting().equals(params.getProxySetting())) {
            if (params.getProxySetting().equals(ProxySetting.NONE)) {
                ...
                Log.d(TAG, "Proxy on " + newConfig.getSSID()
                ...
                try {
                    ...
                    Log.d(TAG, "Wifi connected and proxy set!");
                } catch (Exception e) {
                    Log.d(TAG, "Warning: Wifi is not connected. Check that the " +
    ...
    private void connectToWifiNetwork(ProxyChangeParams params) throws Exception {
        ...
        if (netId < 0) {
            ...
            if (netId < 0) {
                Log.d(TAG, "Having trouble resetting wifi, hard resetting...");
                APL.disableWifi();
                APL.enableWifi();
                try {
                    waitForWifiConnectivity();
